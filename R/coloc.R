...
dt.list[, out2:=paste0(trait2, "---", gsub(":", ".", region1), ".txt")]
...
for(idx in 1:n){
    ...
    if(!file.exists(paste0(out, ".gz"))){
        ...
        #system(paste0("cp ", headout, " ", out, "; export GCS_OAUTH_TOKEN='", token, "'; tabix ", url, " ", region, " >> ", out, "; gzip ", out))
    }
}
        ...
        if(length(cs1) == 0 || length(cs2) == 0 ){
            ...
            if(!is.null(dt.sum)){
                ...
                for(idx in 1:nrow(dt.sum1)){
                    ...
                    idx1 = dt.sum1.cur$idx1
                    idx2 = dt.sum1.cur$idx2
...
if(nrow(dt.coloc) != 0){
    setcolorder(dt.coloc, c("trait1", "trait2", "region1", "region2", "idx1", "idx2"))
}
